...
- **`DnzRowSelector`** (~22 usos): selector relacional con popup de búsqueda sobre un reporte. Params: `Client`, `ReportId`, `SelectedRow:IDinaupRow` + `SelectedRowChanged`, `Label`, `Placeholder`, `Icon`, `IsRequired`, `Disabled`, `AdvancedFilter`, `OnAdd:EventCallback<Guid>`, `OnOpenRecord`, `OpenInWindow=true`, `DefaultID:Guid`, `KeyboardShortcut`.
