...
Date range picker with predefined options and custom range support.

**Parameters:**

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `OnRangeChanged` | `EventCallback<(DateTime From, DateTime To)>` | - | Callback when range changes |
| `DefaultOption` | `string` | `"thisMonth"` | Initial selected option |

**Available options:** `today`, `yesterday`, `thisWeek`, `lastWeek`, `thisMonth`, `lastMonth`, `thisYear`, `last30Days`, `last90Days`, `custom`
...
<DateRangeSelector DefaultOption="last30Days" OnRangeChanged="@HandleRangeChanged" />

@code {
    async Task HandleRangeChanged((DateTime From, DateTime To) range)
    {
...
Card with integrated date range selector, loading state, and period metadata display.
...
| `DefaultRangeOption` | `string` | `"thisMonth"` | Initial date range option |
| `OnRangeChanged` | `EventCallback<(DateTime From, DateTime To)>` | - | Callback when range changes |

**Usage:**

```razor
<DateRangeCard Title="Sales Report" Icon="analytics" Loading="@isLoading" OnRangeChanged="@LoadSalesData">
    <StatsDisplay Statistics="@salesStats" />
</DateRangeCard>

@code {
    bool isLoading = false;
    List<StatsDisplay.StatisticModel> salesStats = new();

    async Task LoadSalesData((DateTime From, DateTime To) range)
    {
        isLoading = true;
        StateHasChanged();

        salesStats = await SalesService.GetStats(range.From, range.To);
